...
#define BUFF_SIZE 512 // nr de elemente din buffer-ul folosit la pipe-uri
...
void info_fis(const char *numeFis, const int f, const char *dirIzolare)
{
    ...
    else
    {
        ...
        if (pid == -1) 
        {
            printf("Eroare la crearea procesului pentru script!\n");
        }
        if (pid == 0) // Proces fiu
        {
            close(fisDes[0]); // Inchide capatul de citire al pipe-ului!

            // Redirectioneaza iesirea standard catre pipe!
            if (dup2(fisDes[1], 1) == -1) 
            {
                printf("Eroare la redirectarea iesirii standard!\n");
                exit(-1);
            }
            ...
            execlp("/bin/bash", "sh", "-c", comanda, NULL);
        ...
        else
        {
            ...
            int biti_cititi;
            while ((biti_cititi = read(fisDes[0], buffer, BUFF_SIZE)) > 0) 
            {
                if (!strstr(buffer, "SAFE")) //comparam buffer-ul pentru a verifica daca fisierul e safe!
                {
    ...
    for(int i = 5; i <= 14; i++)
    {
        ...
        if(err == 0)
        {
            if(verifApar(i, argv, argc) == 0)
            {
                ...
                if(pipe(fileDesP) == -1)
                {
                    printf("Eroare la creearea pie-ului in main!\n");
                    exit(-1);
                }
                ...
                { 
                    ...
                    if (pid == 0)  // Proces fiu
                    {
                        ...
                        close(fileDesP[0]);
                        if (dup2(fileDesP[1], 1) == -1) 
                        {
                            printf("Eroare la redirectare in main!\n");
                            exit(-1);
                        }
                        close(fileDesP[1]);
                    ...
                    else
                    {
                        ...
                        char buffer[512];
                        int biti;
                        while ((biti = read(fileDesP[0], buffer, BUFF_SIZE)) > 0) 
                        {
                            if(buffer[0] == '1')
                                fisCor++;
                        }
                    ...
                    if (WIFEXITED(status)) // verificam modul in care s-a terminat procesul
                    {
                        printf("Procesul cu  PID %d s-a terminat cu codul %d si cu %d fisiere cu potential periculos!\n", pid, WEXITSTATUS(status), fisCor);
